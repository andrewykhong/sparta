...
#include "math.h"
#include "surf_react_prob_multi_mat.h"
#include "input.h"
#include "update.h"
#include "comm.h"
...
SurfReactProbMultiMat::SurfReactProbMultiMat(SPARTA *sparta, int narg, char **arg) :
...
SurfReactProbMultiMat::~SurfReactProbMultiMat()
{
  if (copy) return;
...
int SurfReactProbMultiMat::react(Particle::OnePart *&ip, int, double *,
...
char *SurfReactProbMultiMat::reactionID(int m)
{
  return rlist[m].id;
}
...
double SurfReactProbMultiMat::reaction_coeff(int m)
{
...
int SurfReactProbMultiMat::match_reactant(char *species, int m)
{
...
int SurfReactProbMultiMat::match_product(char *species, int m)
{
...
void SurfReactProbMultiMat::init_reactions()
{
...
void SurfReactProbMultiMat::readfile(char *fname)
{
  int n,n1,n2,eof;
  ...
  while (1) {
    ...
    // read in masks which are used to unique per-grid-cell reaction sets

    word = strtok(NULL," \t\n");
    if (!word) error->all(FLERR,"Invalid reaction style in file");
    r->mask = input->numeric(FLERR,word);
...
int SurfReactProbMultiMat::readone(char *line1, char *line2, int &n1, int &n2)
{
  char *eof;
